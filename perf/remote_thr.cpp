...
int main (int argc, char *argv[])
{
    ...
    for (message_count = 0;; message_count++) {
        ...
        if (rc < 0) {
            if (zmq_errno () == EAGAIN)
                printf ("error in zmq_sendmsg: %s [or the companion utility "
                        "has ended]\n",
                        zmq_strerror (errno));
            else
                printf ("error in zmq_sendmsg: %s\n", zmq_strerror (errno));
        ...
        if (message_count == 10000) {
            printf ("Changing TX timeout to understand when the companion "
                    "utility has ended\n");
            unsigned int newTimeoutMs = 500;
            unsigned int rc = zmq_setsockopt (s, ZMQ_SNDTIMEO, &newTimeoutMs,
                                              sizeof (newTimeoutMs));
            if (rc != 0) {
                printf ("error in zmq_setsockoopt: %s\n", zmq_strerror (errno));
                return -1;
            }
        } /* else if ((message_count % 1000) == 0) {
            //    printf ("mempool msg size: %zu\n", pool.size ());
            unsigned int elapsed_us = zmq_stopwatch_intermediate (watch);
